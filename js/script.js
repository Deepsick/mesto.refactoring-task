...
function createPopup(popupTemplate, form) {
...
function createForm(inputOptions, formOptions) {
	const inputs = inputOptions.map((option) => new InputField(option));
	const form = new Form({ inputs, ...formOptions });
...
function openFormAdd() {
	const form = createForm(
		...
		{ submitText: '+', formName: 'new', submitCallback: submitFormAdd }
	);
...
function openFormProfile() {
	...
	const form = createForm(
		...
		{ submitText: 'Сохранить', formName: 'profile', submitCallback: submitFormProfile }
	);
...
function submitFormProfile(name, job) {
	userInfoName.textContent = name;
	userInfoJob.textContent = job;
	popup.close();
}

function submitFormAdd(name, url) {
